...
def mels_build_multiplexed_X(conf, X_files):
    ...
    XX = []
    for i, file in tqdm(enumerate(X_files)):
        ...
        XX.append(multiplexed)
    return XX

def mels_demux_XX_y(XX, y):
    """De-multiplex data."""
    X = np.concatenate(XX, axis=0)[..., np.newaxis]
    yy = []
    for i,tmp in enumerate(XX):
        yy.append(np.array([y[i]] * tmp.shape[0]))
...
def train_valid_split_multiplexed(conf, XX, y, demux=True, delta_random_state=0):
    ...
    def split_fold(XX, y, fold_list, demux):
        _XX, _y = np.array(XX)[fold_list], y[fold_list]
        if demux:
...
def load_audio_datafiles(conf, X_or_XX_file, y_file, normalize):
    X_or_XX, y = load_npy(conf, X_or_XX_file), \
                 keras.utils.to_categorical(load_npy(conf, y_file), num_classes=conf.num_classes)
    if normalize:
...
def load_datafiles(conf, X_file, y_file=None, normalize=True):
    ...
    if y_file is not None:
        y = keras.utils.to_categorical(load_npy(conf, y_file), num_classes=conf.num_classes)
    if normalize:
        print(' normalize samplewise')
...
def train_classifier(conf, fold, dataset, model=None, init_weights=None, show_detail=False,
    ...
    callbacks = [
        ModelCheckpoint("weights/{epoch:03d}-{loss:.4f}.h5",
                        # str(datapath(conf, conf.best_weight_file)),
    ...
    if not os.path.exists('weights'):
        ...
        for fn in glob('weights/*.*'):
            os.remove(fn)
    if model is None:
        model = create_model(conf, weights=init_weights, show_detail=show_detail)
    # Train model
