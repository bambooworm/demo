...
def model_cnn_alexnet(input_shape, num_classes, time_compress=[2, 1, 1], early_strides=(2,3)):
    ...
    model.add(Flatten())
    model.add(Dense(256, activation='relu', name="dn_1"))
    model.add(Dropout(0.5))
    model.add(Dense(256, activation='relu', name="dn_2"))
    model.add(Dropout(0.5))
    model.add(Dense(num_classes, activation='softmax', name="dn_3"))
    return model

def model_mobilenetv2(input_shape, num_classes, alpha=0.35):
    base_model = MobileNetV2(weights=None, input_shape=input_shape, include_top=False,
                            alpha=alpha, depth_multiplier=0.5)
    x = base_model.output
    x = GlobalAveragePooling2D()(x)
    x = Dense(1024, activation='relu', name="dn_1")(x)
    predictions = Dense(num_classes, activation='softmax', name="dn_2")(x)
...
def create_model(conf, weights=None, show_detail=False):
    if conf.model == 'alexnet':
        ...
        model = model_mobilenetv2(input_shape=conf.dims, num_classes=conf.num_classes, alpha=0.35)
    ...
    #         if not layerName.startswith("dn_"):
